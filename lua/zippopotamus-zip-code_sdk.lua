...
require("zippopotamus-zip-code_types")
...
local ZippopotamusZipCodeSDK = {}
ZippopotamusZipCodeSDK.__index = ZippopotamusZipCodeSDK
...
ZippopotamusZipCodeSDK._make_feature = _make_feature


function ZippopotamusZipCodeSDK.new(options)
  local self = setmetatable({}, ZippopotamusZipCodeSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function ZippopotamusZipCodeSDK:options_map()
...
function ZippopotamusZipCodeSDK:get_utility()
  return Utility.copy(self._utility)
end


function ZippopotamusZipCodeSDK:get_root_ctx()
  return self._rootctx
end


function ZippopotamusZipCodeSDK:prepare(fetchargs)
...
function ZippopotamusZipCodeSDK:direct(fetchargs)
...
function ZippopotamusZipCodeSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function ZippopotamusZipCodeSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "ZippopotamusZipCodeSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function ZippopotamusZipCodeSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function ZippopotamusZipCodeSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "ZippopotamusZipCodeSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function ZippopotamusZipCodeSDK:GetLocationByPostalCode(data)
  ...
  if data == nil then
    if self._get_location_by_postal_code == nil then
      self._get_location_by_postal_code = EntityMod.new(self, nil)
    end
    return self._get_location_by_postal_code
  end
...
function ZippopotamusZipCodeSDK:GetPostalCodesByCity(data)
  ...
  if data == nil then
    if self._get_postal_codes_by_city == nil then
      self._get_postal_codes_by_city = EntityMod.new(self, nil)
    end
...
function ZippopotamusZipCodeSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = ZippopotamusZipCodeSDK.new(sdkopts)
  sdk.mode = "test"
