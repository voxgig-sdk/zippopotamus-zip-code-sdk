...
The Lua SDK for the ZippopotamusZipCode API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:GetLocationByPostalCode(nil):list(nil, nil)
...
local result, err = client:ZippopotamusZipCode(nil):load(
  { id = "test01" }, nil
)
