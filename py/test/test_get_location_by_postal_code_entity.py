...
class TestGetLocationByPostalCodeEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.GetLocationByPostalCode(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.GetLocationByPostalCode(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set ZIPPOPOTAMUS_ZIP_CODE_TEST_GET_LOCATION_BY_POSTAL_CODE_ENTID JSON to run live")
        client = setup["client"]
        ...
        get_location_by_postal_code_ref01_list_result = get_location_by_postal_code_ref01_ent.list(get_location_by_postal_code_ref01_match, None)
...
def _get_location_by_postal_code_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("ZIPPOPOTAMUS_ZIP_CODE_TEST_GET_LOCATION_BY_POSTAL_CODE_ENTID"))
