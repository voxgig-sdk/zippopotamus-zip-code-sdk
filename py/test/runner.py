# ZippopotamusZipCode SDK test runner
...
class ZippopotamusZipCodeTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ZippopotamusZipCodeTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ZippopotamusZipCodeTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ZippopotamusZipCodeTestRunner.getenv("ZIPPOPOTAMUS_ZIP_CODE_TEST_LIVE")
        override = ZippopotamusZipCodeTestRunner.getenv("ZIPPOPOTAMUS_ZIP_CODE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ZippopotamusZipCodeTestRunner.getenv(key)
        ...
        explain = ZippopotamusZipCodeTestRunner.getenv("ZIPPOPOTAMUS_ZIP_CODE_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if ZippopotamusZipCodeTestRunner._test_control is not None:
            return ZippopotamusZipCodeTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ZippopotamusZipCodeTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ZippopotamusZipCodeTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ZippopotamusZipCodeTestRunner.load_test_control()
...
def load_env_local():
    ZippopotamusZipCodeTestRunner.load_env_local()


def env_override(m):
    return ZippopotamusZipCodeTestRunner.env_override(m)


def entity_data(v):
    return ZippopotamusZipCodeTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ZippopotamusZipCodeTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ZippopotamusZipCodeTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ZippopotamusZipCodeTestRunner.load_test_control()


def live_client_options():
    return ZippopotamusZipCodeTestRunner.live_client_options()


def live_delay_ms():
    return ZippopotamusZipCodeTestRunner.live_delay_ms()
