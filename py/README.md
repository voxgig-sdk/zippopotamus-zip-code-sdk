...
The Python SDK for the ZippopotamusZipCode API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.GetLocationByPostalCode(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.ZippopotamusZipCode(None).load(
    {"id": "test01"}, None
)
