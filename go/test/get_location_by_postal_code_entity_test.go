...
func TestGetLocationByPostalCodeEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.GetLocationByPostalCode(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.GetLocationByPostalCode(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		getLocationByPostalCodeRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.get_location_by_postal_code")))
...
func get_location_by_postal_codeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ZIPPOPOTAMUS_ZIP_CODE_TEST_GET_LOCATION_BY_POSTAL_CODE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ZIPPOPOTAMUS_ZIP_CODE_TEST_GET_LOCATION_BY_POSTAL_CODE_ENTID"])
