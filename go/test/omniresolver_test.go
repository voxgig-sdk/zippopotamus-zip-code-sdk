...
import (
	...
	omni "github.com/voxgig-sdk/zippopotamus-zip-code-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ZippopotamusZipCodeError); is {
				out := map[string]any{
					"name":    "ZippopotamusZipCodeError",
...
func omniCtx(arg any, client *sdk.ZippopotamusZipCodeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ZippopotamusZipCodeSDK); is {
			client = live
