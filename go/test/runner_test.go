...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("ZIPPOPOTAMUSZIPCODE_TEST_LIVE") == "TRUE" ||
		os.Getenv("ZIPPOPOTAMUSZIPCODE_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("ZIPPOPOTAMUSZIPCODE_TEST_EXPLAIN"); explain != "" {
