...
func TestGetPostalCodesByCityDirect(t *testing.T) {
	t.Run("direct-list-get_postal_codes_by_city", func(t *testing.T) {
		setup := get_postal_codes_by_cityDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-get_postal_codes_by_city", _mode); _shouldSkip {
			if _reason == "" {
		...
		if setup.live {
			for _, _liveKey := range []string{"city01", "country01", "state01"} {
...
type get_postal_codes_by_cityDirectSetupResult struct {
...
func get_postal_codes_by_cityDirectSetup(mockres any) *get_postal_codes_by_cityDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		return &get_postal_codes_by_cityDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &get_postal_codes_by_cityDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
