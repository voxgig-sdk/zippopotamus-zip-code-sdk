...
// ZippopotamusZipCode SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class ZippopotamusZipCodeTestFeature extends ZippopotamusZipCodeBaseFeature
{
    ...
    public function init(ZippopotamusZipCodeContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (ZippopotamusZipCodeContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = ZippopotamusZipCodeParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (ZippopotamusZipCodeContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(ZippopotamusZipCodeContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = ZippopotamusZipCodeParam::call($ctx, $k);
