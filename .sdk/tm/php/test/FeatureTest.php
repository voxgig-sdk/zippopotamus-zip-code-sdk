...
// ZippopotamusZipCode SDK feature test
//
...
class FtCtrl extends ZippopotamusZipCodeControl
{
...
class FtClient
{
    ...
    private ZippopotamusZipCodeUtility $utility;

    public function __construct(ZippopotamusZipCodeUtility $utility, array $options)
    {
    ...
    public function get_utility(): ZippopotamusZipCodeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ZippopotamusZipCodeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ZippopotamusZipCodeContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ZippopotamusZipCodeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ZippopotamusZipCodeConfig::make_feature($name);
    ...
    private static function buildUrl(ZippopotamusZipCodeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ZippopotamusZipCodeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ZippopotamusZipCodeResponse($fetched);
        }
    ...
    private function fail(ZippopotamusZipCodeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ZippopotamusZipCodeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
