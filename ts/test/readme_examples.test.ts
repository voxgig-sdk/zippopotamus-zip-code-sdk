...
//     (\`new ZippopotamusZipCodeSDK(...)\` / \`ZippopotamusZipCodeSDK.test(...)\` become
//     \`ZippopotamusZipCodeSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ZippopotamusZipCodeSDK'
...
const TEST_SEED = {"entity":{"get_location_by_postal_code":{"test01":{"id":"test01"}},"get_postal_codes_by_city":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ZippopotamusZipCodeSDK(...)\` and any
// \`ZippopotamusZipCodeSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ZippopotamusZipCodeSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ZippopotamusZipCodeSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
