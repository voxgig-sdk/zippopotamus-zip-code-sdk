# zippopotamus-zip-code-mcp

[MCP](https://modelcontextprotocol.io) server exposing the ZippopotamusZipCode SDK as
two agent tools — `zippopotamus-zip-code_list` and `zippopotamus-zip-code_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/zippopotamus-zip-code-mcp)
make build
...
claude mcp add --scope user zippopotamus-zip-code \
  -- /absolute/path/to/zippopotamus-zip-code-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./zippopotamus-zip-code-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// zippopotamus-zip-code_list: first page of records
{ "entity": "get_location_by_postal_code" }
{ "entity": "get_location_by_postal_code", "query": { } }

// zippopotamus-zip-code_load: one record by id
{ "entity": "get_location_by_postal_code", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/zippopotamus-zip-code-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user zippopotamus-zip-code \
     -- "$PWD"/dist/*/zippopotamus-zip-code-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `zippopotamus-zip-code_list` and `zippopotamus-zip-code_load` tools now appear
   in new sessions. Ask the agent to *"list get_location_by_postal_code using zippopotamus-zip-code"*
   and it calls `zippopotamus-zip-code_list` with `{"entity":"get_location_by_postal_code"}`.
...
### Call the `zippopotamus-zip-code_list` tool
...
### Call the `zippopotamus-zip-code_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `zippopotamus-zip-code_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `zippopotamus-zip-code_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./zippopotamus-zip-code-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"zippopotamus-zip-code_load","arguments":{"entity":"get_location_by_postal_code","query":{"id":1}}}}'
```
