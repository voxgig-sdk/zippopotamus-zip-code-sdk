...
//    only - a typed ZippopotamusZipCodeContext there would make every ctx
...
final class ZippopotamusZipCodeOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new ZippopotamusZipCodeContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new ZippopotamusZipCodeSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new ZippopotamusZipCodeResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new ZippopotamusZipCodeError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new ZippopotamusZipCodeResponse($clean['response']);
    ...
    public static function err_from_map($m): ?ZippopotamusZipCodeError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new ZippopotamusZipCodeError(is_string($code) ? $code : '', $msg);
    }
