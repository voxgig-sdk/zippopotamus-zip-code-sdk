...
// ZippopotamusZipCode SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = ZippopotamusZipCodeOmni::makeStructRunner(
    ...
    {
        return ZippopotamusZipCodeOmni::objmodel(ZippopotamusZipCodeOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return ZippopotamusZipCodeOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return ZippopotamusZipCodeOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(ZippopotamusZipCodeOmni::structfix($now, Struct::undef()))
                        !== json_encode(ZippopotamusZipCodeOmni::structfix($val, Struct::undef()))) {
