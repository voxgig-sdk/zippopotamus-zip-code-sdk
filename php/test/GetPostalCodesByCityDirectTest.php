...
class GetPostalCodesByCityDirectTest extends TestCase
{
    public function test_direct_list_get_postal_codes_by_city(): void
    {
        $setup = get_postal_codes_by_city_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-get_postal_codes_by_city", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["city01", "country01", "state01"] as $_liveKey) {
