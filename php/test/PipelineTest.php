...
// ZippopotamusZipCode SDK pipeline test
//
...
{
    private static function utility(): ZippopotamusZipCodeUtility
    {
        return new ZippopotamusZipCodeUtility();
    }
    ...
    private static function ctx(array $over = []): ZippopotamusZipCodeContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ZippopotamusZipCodeContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ZippopotamusZipCodeOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ZippopotamusZipCodeError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ZippopotamusZipCodeOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ZippopotamusZipCodeMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ZippopotamusZipCodeOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ZippopotamusZipCodeMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ZippopotamusZipCodeOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ZippopotamusZipCodeMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ZippopotamusZipCodeSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ZippopotamusZipCodeMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ZippopotamusZipCodeResponse([]),
            'result' => new ZippopotamusZipCodeResult([]),
        ]);
        [, $err] = ZippopotamusZipCodeMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ZippopotamusZipCodeSpec([]),
            'response' => null,
            'result' => new ZippopotamusZipCodeResult([]),
        ]);
        [, $err] = ZippopotamusZipCodeMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ZippopotamusZipCodeSpec(['step' => 's']),
            'response' => new ZippopotamusZipCodeResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ZippopotamusZipCodeResult([]),
        ]);
        [, $err] = ZippopotamusZipCodeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ZippopotamusZipCodeSpec(['step' => 's']),
            'response' => new ZippopotamusZipCodeResponse(self::resp(200, ['v' => 1])),
            'result' => new ZippopotamusZipCodeResult([]),
        ]);
        [, $err] = ZippopotamusZipCodeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ZippopotamusZipCodeSpec(['step' => 's']),
            'response' => new ZippopotamusZipCodeResponse(self::resp(200, ['v' => 2])),
            'result' => new ZippopotamusZipCodeResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ZippopotamusZipCodeMakeResponse::call($ctx);
    ...
    {
        $preset = new ZippopotamusZipCodeResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ZippopotamusZipCodeSpec([]),
            'response' => new ZippopotamusZipCodeResponse([]),
            'result' => new ZippopotamusZipCodeResult([]),
        ...
        [$got, $err] = ZippopotamusZipCodeMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ZippopotamusZipCodeResult([])]);
        [, $err] = ZippopotamusZipCodeMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ZippopotamusZipCodeSpec([]), 'result' => null]);
        [, $err] = ZippopotamusZipCodeMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ZippopotamusZipCodeSpec(['step' => 's']),
            'result' => new ZippopotamusZipCodeResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ZippopotamusZipCodeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ZippopotamusZipCodeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ZippopotamusZipCodeSpec(['step' => 's']),
            'result' => new ZippopotamusZipCodeResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ZippopotamusZipCodeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ZippopotamusZipCodeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ZippopotamusZipCodeResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ZippopotamusZipCodeSpec([]),
            'result' => new ZippopotamusZipCodeResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ZippopotamusZipCodeMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ZippopotamusZipCodeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ZippopotamusZipCodeError('boom', 'boom');
        $utility->fetcher = function (ZippopotamusZipCodeContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ZippopotamusZipCodeSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ZippopotamusZipCodeMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ZippopotamusZipCodeContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ZippopotamusZipCodeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ZippopotamusZipCodeResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        ZippopotamusZipCodeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ZippopotamusZipCodeContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ZippopotamusZipCodeSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ZippopotamusZipCodeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ZippopotamusZipCodeResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ZippopotamusZipCodeSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ZippopotamusZipCodeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ZippopotamusZipCodeMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ZippopotamusZipCodeMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ZippopotamusZipCodeResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ZippopotamusZipCodeDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ZippopotamusZipCodeResult(['ok' => false])]);
        $this->expectException(ZippopotamusZipCodeError::class);
        ZippopotamusZipCodeDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ZippopotamusZipCodeResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ZippopotamusZipCodeMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ZippopotamusZipCodeMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ZippopotamusZipCodeBaseFeature();
        $b = new ZippopotamusZipCodeBaseFeature();
        ZippopotamusZipCodeFeatureAdd::call($ctx, $a);
        ZippopotamusZipCodeFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ZippopotamusZipCodeBaseFeature
    {
        $f = new ZippopotamusZipCodeBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ZippopotamusZipCodeFeatureAdd::call($ctx, self::named_feature('a'));
        ZippopotamusZipCodeFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        ZippopotamusZipCodeFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        ZippopotamusZipCodeFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        ZippopotamusZipCodeFeatureAdd::call($ctx, $replace);
        ...
        ZippopotamusZipCodeFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ZippopotamusZipCodeContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ZippopotamusZipCodeSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ZippopotamusZipCodePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        ZippopotamusZipCodePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ZippopotamusZipCodeResponse(['headers' => null]),
            'result' => new ZippopotamusZipCodeResult([]),
        ]);
        ZippopotamusZipCodeResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        ZippopotamusZipCodeResultBody::call($ctx);
