...
// ZippopotamusZipCode SDK retry feature
...
class ZippopotamusZipCodeRetryFeature extends ZippopotamusZipCodeBaseFeature
{
    ...
    public function init(ZippopotamusZipCodeContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ZippopotamusZipCodeContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ZippopotamusZipCodeContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ZippopotamusZipCodeContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
