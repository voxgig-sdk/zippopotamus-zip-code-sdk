...
class GetLocationByPostalCodeEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "get_location_by_postal_code";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new GetLocationByPostalCodeEntity($this->_client, $opts);
    }

    /**
     * @param GetLocationByPostalCode|array $args GetLocationByPostalCode data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ZippopotamusZipCodeHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GetLocationByPostalCode|array The current GetLocationByPostalCode data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GetLocationByPostalCode fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ZippopotamusZipCodeHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GetLocationByPostalCode fields).
     */
     ...
     *
     * @param GetLocationByPostalCodeListMatch|array|null $reqmatch Match filter (any subset
     *   of GetLocationByPostalCode fields) as an assoc-array; GetLocationByPostalCodeListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GetLocationByPostalCode[]|array A list of GetLocationByPostalCode items as assoc-arrays at
     *   the SDK boundary; throws ZippopotamusZipCodeError on failure (item-5 convention).
     */
