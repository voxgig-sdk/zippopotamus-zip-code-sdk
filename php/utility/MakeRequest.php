...
class ZippopotamusZipCodeMakeRequest
{
    public static function call(ZippopotamusZipCodeContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new ZippopotamusZipCodeResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new ZippopotamusZipCodeResponse($fetched);
        } else {
